...
- Adding or changing entries in `src/runtime/auroraGenRegistry.tsx`
- Changing `GenUIRenderer` DSL wiring — keep `src/runtime/dslRendererWiring.ts` in sync
- Changing lint action prop keys — `GEN_LINT_ACTION_PROP_KEYS` in `src/runtime/genLint.ts`
- Changing component section groupings — `src/runtime/dslRegistryTaxonomy.ts` (`GEN_REGISTRY_TYPE_CATEGORY`)
